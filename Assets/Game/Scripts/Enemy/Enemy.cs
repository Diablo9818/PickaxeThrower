...
public class Enemy : MonoBehaviour
{
    ...
    private float delayForReady = 1f;


    public void Init(AudioService audioService, PointerManager pointerManager)
    {
        _audioService = audioService;
        _enemyPointer.Init(pointerManager);
        StartCoroutine(SetReady(delayForReady));
    }
    ...
    private IEnumerator SetReady(float delayForReady)
    {
        yield return new WaitForSeconds(delayForReady);
        _isReady = true;
    }
}
